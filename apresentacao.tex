...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Transformação de caractere em vetor. \\ Fonte: \textcite{Evelin2017}}
\end{figure}
\end{frame}


\begin{frame}
O problema de classificação trata-se de um problema e programação quadrática convexa com restrições lineares, que pode ser formulado como
\[
\begin{aligned}
\min_{w,b} & \quad f(w,b) \\
...
\begin{frame}
Pelo Lema anterior $\Hset^{+}:=\{x\in \RR^n \mid w^{T}x+b\geq 1\}$ e $\Hset^{-}:=\{x\in \RR^n \mid w^{T}x+b\leq -1\}$ são os hiperplanos que definem a faixa que separa os conjuntos $\Xset^{+}$ e $\Xset^{-}$.
...
\begin{frame}
Este Lema estabelece a largura da faixa entre os hiperplanos separadores $\Hset^{+}$ e $\Hset^{-}$.
